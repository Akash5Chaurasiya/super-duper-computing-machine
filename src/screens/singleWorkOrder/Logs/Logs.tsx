import { ScrollView, StyleSheet, Text, View, FlatList, TouchableOpacity, Modal as Modal1, Modal as Modal2, Modal as Modal3, Image, BackHandler, Alert, TextInput } from 'react-native'
...
import { useGetAllEmployeesQuery, useGetAllMachinesMutation, usePostActiveLogsMutation, usePostChildPartOrderMutation } from '../../../redux/features/apis2/Manufacturing';
import { ActivityIndicator } from 'react-native';

const Logs = ({ id, navigation, uniqueProcess, setLoading1 }: any) => {
    ...
    const filteredTrueEmployees = (allEmployees?.employeeArray)?.filter((machine: any) => machine.active) || []
    console.log(filteredFalseMachines, allMachines);
    ...
    const searchData = (query: any) => {
        if (!query) {
            return filteredTrueEmployees
        }
        query = query.toLowerCase();
        return filteredTrueEmployees.filter((m: any) => {
    ...
    const searchData1 = (query: any) => {
        ...
        return filteredFalseMachines.filter((m: any) => {
            return m.machineName.toLowerCase().includes(query);
        }
        )
    }
    const searched1 = search1 ? searchData1(search1) : filteredFalseMachines;
    const searched = search ? searchData(search) : filteredTrueEmployees;
    ...
    useEffect(() => {
        ...
        fetchData();
    }, [selectedOption, selectedOption3, selectedOption4, navigation, id, selectAll, addActivePostError]);
    useEffect(() => {
        const data = { id, selectedOption, selectedOption3, selectedOption4 }
        addActivePost(data)
    }, [id, selectAll])
    ...
    console.log(confirmIds[0]);
    console.log(ids);
    const handleConfirmData = () => {
        setPostForm({ ...postForm, employeeIds: confirmIds[0] });
        setEmployeeModal(false)
    }
    console.log(postForm);
    const handleItemPress2 = (item: any) => {
        // Toggle the selection state of the item
        if (selectedItems2.includes(item)) {
            setSelectedItems2(selectedItems2.filter((selectedItem: any) => selectedItem !== item));
        } else {
            setSelectedItems2([...selectedItems2, item]);
        }
    };
    const isItemSelected2 = (item: any) => selectedItems2.includes(item);
    const empIds2 = filteredFalseMachines?.map((mac: any) => mac._id)
    const empIDs22 = selectedItems2?.map((item: any) => item._id)
    const confirmIds2 = empIDs22.length != 0 ? [empIDs22, selectedItems2] : [empIds2, filteredFalseMachines]
    console.log(confirmIds2[0]);
    const handleConfirmData2 = () => {
        setPostForm({ ...postForm, machineIds: confirmIds2[0] });
        ...
        <View>
            <View style={{ backgroundColor: 'white', height: responsiveHeight(66) }}>
                                ...
                                <View className="bg-[#F5F5F5] w-full  border-b  rounded-sm border-[#F5F5F5] flex-row p-7 justify-between" >
                                    <View className="flex-col gap-1">
                                        ...
                                        <View>
                                            ...
                                            <TouchableOpacity style={{ backgroundColor: '#283093', paddingVertical: 8, borderRadius: 5, width: '80%', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-around', marginTop: 5 }} onPress={() => { setId1(item.partId), setModalVisible1(true), setPostForm({ ...postForm, workOrderId: id, childPartId: item.partId }), setVal(item) }}>
                        ...
                        <View style={{ backgroundColor: 'white', borderRadius: 10, justifyContent: 'center', alignItems: 'center', width: '90%', paddingHorizontal: 20, borderWidth: 1 }}>
                                ...
                                {val?.contractBased &&
                                    <View style={{ marginBottom: 20, flexDirection: 'row', justifyContent: 'space-between' }}>
                                        <View style={{ marginRight: 10 }}>
                                            <Text style={{ color: "red", textDecorationLine: 'underline' }}>{postForm.employeeIds.length > 0 ? `Selected ${postForm.employeeIds.length}` : "Select Employees"}</Text>
                                            <TouchableOpacity
                                            ...
                                            >
                                                <Text style={{ color: 'white', fontSize: 16 }}>Employees</Text>
                                            </TouchableOpacity>
                                        </View>
                                        <View>
                                            <Text style={{ color: "red", textDecorationLine: 'underline' }}>{postForm.machineIds.length > 0 ? `Selected ${postForm.machineIds.length}` : "Select Machines"}</Text>
                                            <TouchableOpacity
                                                onPress={() => setMachineModal(true)}
                                                style={{
                                                    borderWidth: 1,
                                                    borderColor: '#283093',
                                                    borderRadius: 5,
                                            ...
                                            >
                                                <Text style={{ color: 'white', fontSize: 16 }}>Machines</Text>
                                            </TouchableOpacity>
                                        </View>
                                    </View>}
                                {/* Date Selection */}
                            ...
                            <TouchableOpacity
                                style={{
                                    ...
                                    borderRadius: 10,
                                    marginVertical: 20,
                                    backgroundColor: val?.contractBased && postForm.employeeIds.length <= 0 ? '#dddddd' : '#283093',
                                }}
                                disabled={val?.contractBased && postForm.employeeIds.length <= 0 ? true : false}
                ...
                <Modal3
                    ...
                    transparent={true}
                    animationType="slide"
                    onRequestClose={() => setMachineModal(false)} >
                    <View style={{ backgroundColor: 'white', marginTop: '20%', borderWidth: 0.2, elevation: 9, padding: '3%', marginLeft: '10%', marginRight: '5%' }}>
                        <Feather
                            ...
                            color={'#949494'}
                            style={{ marginLeft: '90%' }}
                            onPress={() => setMachineModal(false)}
                        />
                        <View style={{ height: '70%' }}>
                            <Text style={{ color: '#2E2E2E', fontWeight: '700', fontSize: 20 }}> Machine </Text>
                            <TextInput
                                style={{
                                    borderWidth: 1,
                                    borderColor: '#ccc',
                                        ...
                                        <TouchableOpacity
                                            style={[
                                                styles.card,
                                                {
                                                    ...
                                                    backgroundColor: isItemSelected2(item) ? 'lightblue' : 'transparent'
                                                },
                                            ]}
                                            key={index}
                                            onLongPress={() => handleItemPress2(item)}
                                        >
                                            <View>
                                            ...
                                            <View>
                                                ...
                                                >
                                                    {item?.machineName?.length > 15 ? item?.machineName.substring(0, 15) + ' ...' : item.machineName}
                                                </Text>
                                            </View>
                                            <View >
                                                <Checkbox.Android
                                                    status={isItemSelected2(item) ? 'checked' : 'unchecked'}

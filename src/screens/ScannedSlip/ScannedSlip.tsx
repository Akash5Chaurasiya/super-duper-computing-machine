...
const ScannedSlip = ({ navigation }: any) => {
    ...
    const checkAllFields = () => {
        ...
        setIsButtonDisabled((hasBasicFields && hasAdditionalFields) ? false : true);
    };
    useEffect(() => {
        checkAllFields();
        ...
        dispatch(allotWorkAsync(allot)).then((response: any) => {
            if (response.payload.success == true) {
                ...
                Dialog.show({
                    ...
                    title: "Failure",
                    textBody: response.payload.message + (response.payload.data ? response.payload.data : ''),
                    button: 'close',
                })
            }
        }).catch((error: any) => {
